...
5. Spawn `audit-insights-stager` → `findings.json` + `deck-overrides.json` (verdict, two scorecards, the prioritized fix list **phrased from** the strategist's fixes, and the `element_rationales` Strengths/Gaps lines). Gate the copy twice: (a) `node prose-lint.mjs companies/<slug>/deck-overrides.json` — em dash fails (exit 1); scorer-voice and ellipsis-truncation WARN. (b) **Fact-check:** spawn `audit-copy-verifier` → `companies/<slug>/copy-review.json`, which traces every client-facing claim back to `levers.json` / `metrics.json` / `fixes.json` / the ledgers and flags what is not supported (invented numbers, invented causation, strength drift, unsupported named targets). If either gate flags a line, have the stager fix it before continuing.
