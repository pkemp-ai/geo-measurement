...
import { ELEMENTS, IMPORTANCE, LOCAL_SIGNAL, VALIDATION_SOURCES, VALIDATION_DOMAINS, signalMode } from "./rubric.mjs";
...
// Live rubric tier by element id. Importance resolves tier from the rubric (not the
// tier snapshotted into levers.json at score time) so re-running the importance stage
// picks up tier/classification changes without re-judging.
const TIER = Object.fromEntries(ELEMENTS.map((e) => [e.id, e.tier]));
...
export function computeImportance(result, { classified, graded, profiles }) {
  ...
  for (const [lever, obj] of Object.entries(result.levers)) {
    for (const [id, e] of Object.entries(obj.elements)) {
      ...
      const liveTier = TIER[id] ?? e.tier;
      e.tier = liveTier; // sync stored tier to the live rubric (re-run over an older rubric)
      const g = prior(liveTier);
      const mode = signalMode(id);
      e.signal = mode;
      e.importance = {}; e.local_evidence = {};
      let best = 0;
      for (const track of tracksFor(e.job)) {
        const ls = localSignal(id, sigs[track], { graded });
        // "prior" elements (gates, grounding, offsite awareness) are not observable
        // as cited sources in the measured prompts, so importance = theory prior only;
        // the observed share is still recorded in local_evidence for transparency.
        const raw = mode === "prior" ? g : (IMPORTANCE.w_global * g + IMPORTANCE.w_local * ls.share);
...
export function buildImportanceMatrix(result) {
  ...
  for (const [lever, obj] of Object.entries(result.levers)) {
    for (const [id, e] of Object.entries(obj.elements)) {
      const tier = TIER[id] ?? e.tier;
      const mode = signalMode(id);
      const research = round1(1 + 4 * prior(tier));
      const indeterminate = isIndeterminate(e);
      ...
      for (const track of tracksFor(e.job)) {
        ...
        rows.push({
          ...
          // run importance only means something for citation/harm modes; for prior
          // elements the observed share is not a valid signal, so it is null.
          run_importance: mode === "prior" ? null : round1(1 + 4 * (ls.share ?? 0)),
